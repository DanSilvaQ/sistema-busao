package com.empresa.transportebusao.ratelimit;
...
public class RateLimitFilter implements ContainerRequestFilter, ContainerResponseFilter {
    ...
    public void filter(ContainerRequestContext requestContext) throws IOException {
        ...
        // Aplica rate limit apenas no endpoint /api/v1/motoristas
        String path = requestContext.getUriInfo().getPath();
        System.out.println("=== Path da requisição: " + path);

        if (!path.startsWith("/api/v1/motoristas")) {
            System.out.println("=== Path não começa com 'api/v1/motoristas', pulando rate limit...");
            return;
        }

        System.out.println("=== Aplicando rate limit em motoristas...");
